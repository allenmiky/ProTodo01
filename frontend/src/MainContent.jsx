...
import { FiChevronLeft, FiChevronRight, FiArchive, FiSun, FiMoon, FiPlus, FiTrash2, FiCalendar } from "react-icons/fi";
...
export default function MainContent() {
 const [user, setUser] = useState(() => {
  ...
  if (!user) return <AuthPage darkMode={dark} onLogin={handleLogin} />;
  ...
  if (loading)
    return (
      ...
      >
        <div className="flex text-5xl md:text-7xl font-bold select-none">
          <span
  ...
  return (
    ...
    >
      ...
      {/* HEADER */}
      <header
        className={`h-14 px-6 flex items-center justify-between ${dark ? "bg-gray-900 border-gray-800" : "bg-white border-b border-sky-100"
          }`}
      >
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`px-3 py-1 rounded-lg text-sm flex items-center ${dark ? "bg-gray-800" : "bg-sky-100"
              }`}
          >
      ...
      <div className="flex flex-1 overflow-hidden">
        {sidebarOpen && (
          <Sidebar
            ...
            addBoard={() => setShowAddBoardModal(true)} // 👈 updated line
